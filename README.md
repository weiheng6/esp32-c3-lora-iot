...
| 📶 **WiFi 扫描** | 支持扫描并选择 WiFi 网络，无需手动输入 SSID |
| 🔋 **智能 AP 模式** | WiFi 连接成功后自动关闭 AP 节省电量，无网络时自动开启 AP |
| 📡 **MQTT 通信** | 支持远程数据上报和指令控制，支持 JSON 和 Protobuf 双协议 |
...
| 类型 | 说明 | 返回内容 |
|------|------|----------|
| `relay` | 继电器状态 | 继电器开关状态、手动模式标志 |
| `condition` | 条件控制 | 条件控制配置详情 |
| `timer` | 定时控制 | 定时控制配置详情 |
| `network` | 网络状态 | WiFi/MQTT连接状态、RSSI、IP地址 |
| `device` | 设备信息 | 设备ID、运行状态、内存信息 |
| `sensor` | 传感器数据 | 实时温湿度数据 |
| `all` | 全部状态 | 以上所有内容 |
...
| 场景 | 响应内容 |
|------|----------|
| 条件控制启用/禁用 | `{"status":"ok","condition_enabled":true}` |
| 继电器手动控制 | `{"status":"ok","relay":1}` |
| 继电器自动控制 | `{"status":"ok","relay":1,"reason":"auto"}` |
| 状态查询 | `{"status":"ok",..."relay":{...},"condition":{...}}` |
...
**AP 模式**：
1. 设备启动后会自动创建 WiFi 热点
2. 连接热点（默认：`Scott_Device`，密码：`88888888`）
3. 打开浏览器访问 `http://192.168.4.1`

**Station 模式**：
1. 设备连接 WiFi 后，访问设备获得的 IP 地址
2. 可以通过路由器查看设备 IP，或从 MQTT 上报数据中获取

### 功能页面

| 页面 | 功能 |
|------|------|
| 仪表盘 | 实时显示温湿度、继电器状态、网络状态、定时任务详情 |
| 📡 网络配置 | WiFi 配置（支持扫描）、MQTT 配置、传输模式设置 |
| 🎛️ 控制策略 | 条件控制和定时控制配置 |
| 🔧 设备配置 | 引脚映射配置、系统信息 |
| 🔄 OTA升级 | Web URL升级、文件上传、MQTT远程升级 |
| 📊 系统监控 | 内存使用、网络诊断 |
...
设备支持扫描周围的 WiFi 网络，方便用户选择：

1. 进入"📡 网络配置"页面
2. 点击 WiFi SSID 输入框旁边的"🔍 扫描"按钮
3. 在弹出的列表中选择要连接的 WiFi 网络
4. 输入密码，点击"💾 保存WiFi"按钮
5. 设备会自动连接并保存配置

### 智能 AP 模式管理

- ✅ **WiFi 连接成功**：自动关闭 AP 模式，节省电量
- ✅ **WiFi 配置更新**：立即尝试连接新配置的 WiFi
- ✅ **WiFi 连接失败**：自动开启 AP 模式，允许重新配置
- ✅ **WiFi 未配置**：自动开启 AP 模式，等待用户配置
...
| 配置项 | 保存按钮 | 说明 |
|--------|---------|------|
| WiFi SSID 和密码 | 💾 保存WiFi | 配置后自动连接 |
| MQTT 配置 | 💾 保存MQTT | 服务器、端口、用户名、密码、Topic |
| MQTT 启用状态 | 自动保存 | 开关切换时即时生效 |
| 传输模式 | 自动保存 | 选择后即时生效 |
| LoRa 启用状态 | 自动保存 | 开关切换时即时生效 |

### 配置同步

Web UI 配置系统（ConfigAPI）和 WiFi 连接管理器（WiFiManager）实时同步：
- 保存配置时同时更新两个系统
- 启动时自动同步两系统的配置
- 配置变更后自动触发重连
...
| WiFi扫描无响应 | AP模式未切换 | 确认设备支持STA模式 |
| 定时任务删除后恢复 | 保存失败 | 检查JSON buffer大小 |
| 设备重启 | 内存不足 | 减小StaticJsonDocument大小 |
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v1.0.0 | 2026-05 | 初始版本，支持温湿度采集、WiFi连接、MQTT通信、继电器控制 |
| v1.1.0 | 2026-05 | 添加条件控制（滞回模式）和定时控制（8组时间段） |
| v1.2.0 | 2026-05 | 添加OTA升级功能：Web URL升级、文件上传、MQTT远程升级 |
| v1.2.1 | 2026-05 | 修复OTA升级进度显示问题，添加详细的升级状态反馈 |
| v1.2.2 | 2026-05 | 优化Web界面OTA文件上传逻辑，修复UPLOAD_FILE_START未触发问题 |
| v1.3.0 | 2026-05 | 添加故障恢复机制和系统监控功能 |
| v1.4.0 | 2026-05 | 添加MQTT状态查询功能，支持查询继电器、条件控制、定时控制、网络状态、设备信息、传感器数据 |
| v1.5.0 | 2026-05 | 添加WiFi扫描功能、智能AP模式管理、ConfigAPI与WiFiManager同步优化、定时控制BUG修复（删除、清空、保存） |
